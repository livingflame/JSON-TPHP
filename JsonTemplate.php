 ...
 */
class JsonTemplateErrorHandler extends Exception
{
...
*/
class JsonTemplateCompilationError extends JsonTemplateErrorHandler
{
 ...
 * the formatters.
 */
class JsonTemplateEvaluationError extends JsonTemplateErrorHandler
{
 ...
 */
class JsonTemplateBadFormatterError extends JsonTemplateCompilationError
{
 ...
 */
class JsonTemplateMissingFormatterError extends JsonTemplateCompilationError
{
 ...
 */
class JsonTemplateSyntaxError extends JsonTemplateCompilationError
{
 ...
 */
class JsonTemplateUndefinedVariableError extends JsonTemplateCompilationError
{
 ...
 */
abstract class JsonTemplateCallbackAbstract
{
	abstract public function call();
}

// calls a function passing all the parameters
class JsonTemplateFunctionCallback extends JsonTemplateCallbackAbstract
{
	protected $function = '';
...
class JsonTemplateStackCallback extends JsonTemplateCallbackAbstract
{
...
class JsonTemplateProgramBuilder
{
	...
	*/
	public function __construct($more_formatters)
	{
...
class JsonTemplateScopedContext implements Iterator
{
	...
	public function __construct($context,$undefined_str=null)
	{
	...
	public function undefined($name) {
		if ($this->undefined_str === null) {
			throw new JsonTemplateUndefinedVariableError(sprintf('%s is not defined',$name));
		} else {
	...
	// context could be an associative array or a StdClass object
 	public function lookup($name) {
		...
		$parts = explode('.',$name);
		$value = $this->lookUpStack($parts[0]);
	...
	public function lookUpStack($name)
	{
...
abstract class JsonTemplateFormatterAbstract
{
	public $context;
	public $func;
	public $args = array();
	...
	public function call($val){
		...
		$method_exists = method_exists(get_class($this),$this->func);
		$method_callable = is_callable(array($this, $this->func));
		if($method_exists && $method_callable){
			return call_user_func_array(array($this,$this->func),$args);
		} elseif(is_callable($this->func,$args)) {
...
class JsonTemplateGenericFormatter extends JsonTemplateFormatterAbstract
{

}

class JsonTemplateHtmlFormatter extends JsonTemplateFormatterAbstract
{
	public function format($str)
	{
...
class JsonTemplateHtmlAttributeValueFormatter extends JsonTemplateFormatterAbstract
{
	public function format($str)
	{
...
class JsonTemplateRawFormatter extends JsonTemplateFormatterAbstract
{
	public function format($str)
	{
		return $str;
...
class JsonTemplateStringFormatter extends JsonTemplateFormatterAbstract
{
	public function format($str)
	{
		if ($str===null)
...
class JsonTemplateSizeFormatter extends JsonTemplateFormatterAbstract
{
...
class JsonTemplateUrlParamsFormatter extends JsonTemplateFormatterAbstract
{
...
class JsonTemplateUrlParamValueFormatter extends JsonTemplateFormatterAbstract
{
...
class JsonTemplatePluralizeFormatter extends JsonTemplateFormatterAbstract
{
	public $args  = array(NULL,'es');
	public function __construct() {
		$this->func = 'pluralize';
	}
	public function addArgs($args = array()){
...
class JsonTemplateTemplateFormatter extends JsonTemplateFormatterAbstract
{
	public $module;
	public function __construct() {
		$this->func = 'template';
		$this->module = JsonTemplateModule::pointer();
	}
...
class JsonTemplateModule
{
	...
	public $default_formatters = array(
		'html'				=> 'JsonTemplateHtmlFormatter',
		'html-attr-value'	=> 'JsonTemplateHtmlAttributeValueFormatter',
		'htmltag'			=> 'JsonTemplateHtmlAttributeValueFormatter',
		'raw'				=> 'JsonTemplateRawFormatter',
		'size'				=> 'JsonTemplateSizeFormatter',
		'url-params'		=> 'JsonTemplateUrlParamsFormatter',
		'url-param-value'	=> 'JsonTemplateUrlParamValueFormatter',
		'str'				=> 'JsonTemplateStringFormatter',
		'default_formatter'	=> 'JsonTemplateRawFormatter',
		'pluralize'			=> 'JsonTemplatePluralizeFormatter',
		'template'			=> 'JsonTemplateTemplateFormatter',
	);
	...
	/*
	  ...
	  Throws:
	    ...
	    JsonTemplateMissingFormatterError is raised.
	...
	*/
	...
	public function compileTemplate($template_str, $options=array(), $builder=null)
	{
			...
			if(($i % 2) == 0){
				...
				if(preg_match($this->template_re,$token,$match)){
					if($definition){
						...
						$this->compileTemplate($this->other_templates[$match[2]], $options, $builder);
					}
				...
				if($token == 'end'){
					...
					if($balance_counter < 0){
						...
						throw new JsonTemplateSyntaxError(sprintf(
				...
				if($definition){
					...
					if(count($parts) == 1){
						if(!$options['default_formatter']){
							throw new JsonTemplateMissingFormatterError('This template requires explicit formatters.');
		...
		if($balance_counter != 0){
			throw new JsonTemplateSyntaxError(sprintf('Got too few %send%s statements', $meta_left, $meta_right));
		}
		return $builder->root();
	...
	*/
	...
	public function fromString($string, $constructor='JsonTemplate')
	{
		$options = array();
		$lines = explode("\n",$string);
	...
	public function doRepeatedSection($block, $context, $callback)
	{
		...
		if($items){
			...
			foreach($context as $i=>$data){
				...
				if($i != $last_index){
					$this->execute($alt_statements, $context, $callback,'alternates with');
				}
			}
	...
	public function doSubstitute($name, $formatters, $context, $callback=null)
	{
		...
		if($name == '@'){
			...
			}catch(JsonTemplateUndefinedVariableError $e){
				throw $e;
			}catch(Exception $e){
		...
		if($callback instanceof JsonTemplateCallbackAbstract){
	...
	/*
	 * execute a bunch of template statements in a JsonTemplateScopedContext.
  	 * Args:
         * callback: Strings are "written" to this callback function.
	 *
  	 * This is called in a mutually recursive fashion.
	 */
	public function execute($statements, $context, $callback,$name = '')
	{
		//echo \Debug::dump($statements,$name);
		...
		foreach($statements as $i=>$statement){
			if(is_string($statement)){
				...
				try{
					if($statement instanceof JsonTemplateCallbackAbstract){
	...
	*/
	static function expand($template_str, $data, $options=array())
	{
...
class JsonTemplate
{
	...
	*/
	public function __construct($template_str, $options=array(), $builder=null)
	{
	...
	*/
	public function tokenstream($data)
	{
